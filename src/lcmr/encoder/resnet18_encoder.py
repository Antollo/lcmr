...
class ResNet18Encoder(PretrainedEncoder):
    def __init__(
        ...
        model = resnet18(weights=weights, replace_stride_with_dilation=replace_stride_with_dilation)
        model = nn.Sequential(*list(model.children())[:-2])

        super().__init__(model, input_size=input_size, frozen=frozen)

    def forward(self, x: ImageBHWC3) -> TensorType[batch_dim, 512, reduced_height_dim, reduced_width_dim, torch.float32]:
